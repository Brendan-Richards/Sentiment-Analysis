...
def run_gridsearches(
    ...
    for i in range(len(pipelines)):
        ...
        if run_type == "clas":
            print(
                "accuracy score: "
                + str(accuracy_score(gs.best_estimator_.predict(dtestx), dtesty))
            )
            ...
            print(
                "mean_absolute_error: "
                + str(mean_absolute_error(gs.best_estimator_.predict(dtestx), dtesty))
            )
            print(
                "mean_squared_error: "
                + str(mean_squared_error(gs.best_estimator_.predict(dtestx), dtesty))
            )
            print(
                "guesses in correct range: "
                + str(
                    accuracy_score(
                        classify_regressor_predictions(
                            gs.best_estimator_.predict(dtestx)
                        ),
                        dtesty,
                    )
                )
